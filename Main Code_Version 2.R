...
k.rfdp2.folds <- function(k) {
  ...
  for (i in 1:k) {
    ...
    predictions1 <- predict(model1,newdata = bank2[-folds[[i]],],type="prob")
    predictions2 <- predict(model2,newdata = bank2[-folds[[i]],],type="prob")
    valid_all <- (predictions1[,1] + predictions2[,1]) / 2
    valid_all <- as.data.frame(valid_all)
    mccrs = c(mccrs, mccr(as.integer(as.logical(bank2[-folds[[i]], ]$y=='yes')),
                          as.integer(as.logical(valid_all[,1]>0.5))))
...
mccrsRFDP2
mean(mccrsRFDP2)
# [1] 0.4668281 0.5231650 0.5218441 0.5416872 0.5281949 0.5427251 0.4145878 0.5116835 0.5389919 0.4839584 0.5405930 0.5404638 0.5150919
# [14] 0.4990682 0.4989255 0.5003861 0.5220050 0.5136398 0.5077093 0.4942328 0.5380141 0.4930252 0.4987290 0.5090411 0.5274406
#------------------------------------------------------------------------------------
